...
Write-Host "🏛️  Starting Automated Deployment (Corinthian Stack) 🏛️" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan
...
if (-not (Test-Path ".\cert.pem") -or -not (Test-Path ".\key.pem")) {
    Write-Host "⚠️  Certificates missing. Generating new local SSL certs via mkcert..." -ForegroundColor Gray
    try {
...
try {
    if (Test-Path ".\wordpress-plugins.json") {
        ...
        foreach ($plugin in $wpPlugins) {
            Write-Host "   ⚙️  Installing and activating: $($plugin.slug)" -ForegroundColor Gray
    ...
    Write-Warning "⚠️  Warning: WordPress configuration failed. Ensure WP initial configuration is done in the browser." 
}
...
try {
    ...
    Write-Warning "⚠️  Warning: MediaWiki asset configuration failed. Reason: $_" 
}

# ---------------------------------------------------------
# Phase 5: NodeBB Plugin Activation & Asset Compilation
# ---------------------------------------------------------
Write-Host "🛠️  [5/6] Activating and compiling NodeBB SSO module..." -ForegroundColor Yellow
try {
    ...
    Write-Host "   ♻️  Executing soft restart on NodeBB service instance..." -ForegroundColor Gray
    docker compose exec -T nodebb ./nodebb restart
    Write-Host "✅ NodeBB instance compiled successfully!" -ForegroundColor Green
} catch { 
    Write-Warning "⚠️  Warning: NodeBB compilation failed. Try running './nodebb build' manually." 
}
